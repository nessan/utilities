...
two replacements for the standard [`assert`] macro --- they both allow for an additional string output that you can use to print the values of the variables that triggered any failure.
...
3. This macro expands to `verify` **unless** the `NDEBUG` flag is set in which case it's a no-op. This is similar to the standard [`assert`] macro.

Assuming a check is "on", in all cases, if `condition` evaluates to `false`, these all call the macro:
```cpp
exit_with_message(message)
```
The `exit_with_message(message)` macro passes the `message` arguments and the source code location of the failure to `utilities::exit(...)`.
...
A primary use case for `debug_verify` is to do things like bounds checking on indices --- from experience, this is vital during development.
