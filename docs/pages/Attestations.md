...
The `<utilities/attest.h>` header includes macros for checking a boolean condition or comparing two values. <br>
...
Failed attestions show the source code location where the attestion was called, along with a stringified version of the condition that caused the failure.
...
```cpp
always_attest(condition, ...)   // <1>
debug_attest(condition, ...)    // <2>
attest(condition, ...)          // <3>
```
...
2. The `debug_attest` macro expands to `always_attest` **if** the `DEBUG` flag is set; otherwise, it's a no-op.
3. The `attest` macro expands to `always_attest` **unless** the `NDEBUG` flag is set, which is a no-op. <br>This is similar to the [standard `assert`] macro.
...
```cpp
always_attest_eq(lhs, rhs, ...) // <1>
debug_attest_eq(lhs, rhs, ...)  // <2>
attest_eq(lhs, rhs, ...)        // <3>
```
...
2. The `debug_attest_eq` macro expands to `always_attest_eq` **if** the `DEBUG` flag is set; otherwise, it's a no-op.
3. The `attest_eq` macro expands to `always_attest_eq` **unless** the `NDEBUG` flag is set, which is a no-op similar to the [standard `assert`] macro.
...
For the sake of the example, we will use the `always_attest_eq` macro to check for equality.
More typically, you use either the `debug_attest_eq` or the `attest_eq` macro.

```cpp
#include <utilities/attest.h>
int subtract(int x, int y)
{
    always_attest_eq(x, y, "well that didn't work but x - y = {}", x - y);
    return y - x;
}
int main()
{
...
```text
FAILED `attest_eq(x, y)` [attest02.cpp:7]
...
The `always_attest` and `always_attest_eq` macros are used to verify that a condition is true and to exit with an informative message if it is not. <br>
...
The `debug_attest` and `debug_attest_eq` macros cover this type of verification.
...
```cpp
debug_attest_eq(u.size(), v.size(), "Incompatible vector sizes!");
```
...
On the other hand, there are some checks where the attestion cost is slight compared to the function's overall work.
...
```cpp
attest(M.is_square(), "Cannot invert a {} x {} NON-square matrix!", M.rows(), M.cols());
```
...
A primary use case for `debug_attest` and `debug_attest_eq` is checking index bounds.
...
The main reason we use macros instead of a function is that we need to stringify the attestion condition and attestion arguments to produce a helpful message if the attestion fails.
