...
2. Returns a string for the type of a specific object, so if `x` is an `int`, then `utilities::type(x)` will return `"int"`.
...
However, that `name` field is not standardized across compilers, and in some cases, it contains the mangled name, which is not easily readable
Moreover, [`std::typeid`] is not a compile-time call, so no use for template meta-programming.
...
```
1. See [`stopwatch.h`] for details.
2. See [`macros.h`] for details.
...
We conclude that all three use the default specified in [`stopwatch.h`], `std::chrono::high_resolution_clock`.
...
The standard library `libc++` for `clang` seems to be able to access two different clocks (or at least two that it thinks are different).
